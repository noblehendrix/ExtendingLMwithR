# A useful simulation script for evaluating the binomial model
# developed by Cole Monnahan |  monnahc@uw.edu
...
plot.binomial.glm <- function(num.trials, beta0, beta1,
  ...
  if(make.random){
    ...
    p.obs <- sapply(1:length(X.obs), function(i) rbinom(n=1, size=num.trials,prob=p.true.obs[i]))/num.trials
